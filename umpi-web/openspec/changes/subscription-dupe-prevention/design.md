...
Conflict = `subscription_authorized_payment` for P with a live `(user, plan)` row on a different preapproval. Pre-check (excluding same preapproval id) + `23505` on the upsert as race backstop. **Decision 6.2 (RESOLVED — see Open Questions)**: when the winner is still `pending` and P is `authorized`, the user paid through P → **promote P** instead of cancelling it: cancel the pending winner FIRST (MP + DB — the only sequence that frees the live slot before P activates, avoiding 23505), upsert P to `active`, then converge the profile with the same paid-state write as the happy path (gate-review fix: promote previously left the profile on 'trial'). Profile-convergence failure acks 200 `retry_via_cron`, never 5xx (P is active, not a marker row — the cron cannot cancel it; it converges later via sync-subscription or P's next webhook event). All other conflicts follow the cancel-P flow:
...
- [x] Edge: winner row still `pending` while conflicting P is `authorized` — **RESOLVED (product decision 2026-08-09): PROMOTE P** (preserve the user's payment). `promoteP` cancels the unpaid pending winner first (MP + DB — the only sequence satisfying the live-slot index), activates P via upsert keyed by `mp_preapproval_id`, then converges the profile with the same paid-state write as the happy path; a profile-convergence failure acks 200 `retry_via_cron` (never 5xx — P is active, not a marker row, so the cron cannot cancel it; it converges later via sync/webhook). The two handled promote 500s (winner-without-MP-id, non-23505 upsert failure) remain deliberate — see the mp-webhook header comment.
